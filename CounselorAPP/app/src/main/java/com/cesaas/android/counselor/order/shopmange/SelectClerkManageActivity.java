 ...
 * 店员管理列表
 */
public class SelectClerkManageActivity extends BasesActivity implements View.OnClickListener{
    ...
    public void onEventMainThread(ResultAgreeClerkBean bean) {
        ...
        if(bean.IsSuccess==true){
            ToastFactory.getLongToast(mContext,"该店员启用成功");
            mCounselorListNet.setData(pageIndex,1,"");
        }else{
            ToastFactory.getLongToast(mContext,"该店员启用失败"+bean.Message);
        }
    }
...
//
    ...
    private OnItemClickListener onItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(int position) {
            ...
            bean.setSalesId(mCounselorListBeanList.get(position).getCOUNSELOR_ID());
            if(mCounselorListBeanList.get(position).getCOUNSELOR_NAME()!=null){
                bean.setSalesName(mCounselorListBeanList.get(position).getCOUNSELOR_NAME());
            }else{
                bean.setSalesName(mCounselorListBeanList.get(position).getCOUNSELOR_NICKNAME());
            }
            Intent mIntent = new Intent();
            mIntent.putExtra("selectList",bean);
