...
import com.cesaas.android.java.adapter.move.MoveDeliveryGoodsDetailScanAdapter;
...
import com.cesaas.android.java.net.move.MoveDeliveryChangeByBarcodeNoNet;
import com.cesaas.android.java.net.move.MoveDeliveryDeleteByBarcodeNoNet;
 ...
 *
 */
public class ZxingScanBoxActivity extends BasesActivity implements QRCodeView.Delegate ,View.OnClickListener{
	...
	private MoveDeliveryGoodsDetailScanAdapter adapter;
	...
	private MoveDeliveryChangeByBarcodeNoNet moveDeliveryChangeByBarcodeNoNet;
	private MoveDeliveryDeleteByBarcodeNoNet moveDeliveryDeleteByBarcodeNoNet;
	...
	public void onEventMainThread(ResultMoveDeliveryGoodsDetailBean msg) {
		if(msg.arguments.resp.errorCode!=0){
			if(msg.arguments.resp.records.value!=null && msg.arguments.resp.records.value.size()!=0){
				...
				adapter=new MoveDeliveryGoodsDetailScanAdapter(mData,mContext);
					...
					if (materialDialog != null) {
						materialDialog.setTitle("温馨提示！")
								...
								.setPositiveButton("确定", new View.OnClickListener() {
									@Override
									public void onClick(View v) {
										moveDeliveryDeleteByBarcodeNoNet=new MoveDeliveryDeleteByBarcodeNoNet(mContext,type);
			...
			new AsyncTask<Void, Void, String>() {
				...
				protected void onPostExecute(String result) {
					if (TextUtils.isEmpty(result)) {
						Toast.makeText(ZxingScanBoxActivity.this, "未发现二维码", Toast.LENGTH_SHORT).show();
					} else {
						//执行扫描盘点操作
						moveDeliveryScanNet=new MoveDeliveryScanNet(mContext,type);
						moveDeliveryScanNet.setData(pid,boxId,result,num);
					}
				}
			}.execute();
		}
			...
			switch (view.getId()) {
				...
				case R.id.tv_sure:
					if(!TextUtils.isEmpty(et_goods_num.getText().toString())){
						...
						moveDeliveryChangeByBarcodeNoNet=new MoveDeliveryChangeByBarcodeNoNet(mContext,type);
