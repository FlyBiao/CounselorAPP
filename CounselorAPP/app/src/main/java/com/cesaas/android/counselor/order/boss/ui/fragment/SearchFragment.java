...
/**
 * Demo: 搜索结果显示Fragment
 * Created by FlyBiao on 17/08/15
 */
public class SearchFragment extends Fragment {
    ...
    public void bindDatas(List<ShopListBean> datas) {
        this.mDatas = datas;
    ...
    private class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.VH> implements Filterable {
        ...
        public VH onCreateViewHolder(ViewGroup parent, int viewType) {
            final VH holder = new VH(LayoutInflater.from(getActivity()).inflate(R.layout.item_shop_index, parent, false));
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int position = holder.getAdapterPosition();
                    ToastFactory.getLongToast(getContext(),"选择了" + items.get(position).getNick());
                }
            });
            return holder;
        }
        ...
        public void onBindViewHolder(VH holder, int position) {
            holder.tvName.setText(items.get(position).getNick());
        }
        ...
        public Filter getFilter() {
            return new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence constraint) {
                    ...
                    for (ShopListBean item : mDatas) {
                        if (item.getPinyin().startsWith(constraint.toString()) || item.getNick().contains(constraint)) {
                            list.add(item);
                        }
                    }
                ...
                protected void publishResults(CharSequence constraint, FilterResults results) {
                    ArrayList<ShopListBean> list = (ArrayList<ShopListBean>) results.values;
                    items.clear();
                    items.addAll(list);
                    if (results.count == 0) {
