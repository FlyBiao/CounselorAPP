...
import com.cesaas.android.counselor.order.member.adapter.service.apply.MemberCompleteInfoAdapter;
...
public class CompleteMemberInfoFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,BaseQuickAdapter.RequestLoadMoreListener{
    ...
    private MemberCompleteInfoAdapter adapter;
    private List<MemberApplyListBean> mData=new ArrayList<>();

    /**
     * 单例
     */
    public static CompleteMemberInfoFragment newInstance() {
        CompleteMemberInfoFragment fragmentCommon = new CompleteMemberInfoFragment();
    ...
    public void initData() {
        net=new VipDataListNet(getContext(),2);
    ...
    public void onEventMainThread(SearchVipEventBean msg) {
        if(msg.getType()==20){
            ...
            net.setData(pageIndex,20,msg.getContent());
        }
    }

    public void onEventMainThread(ResultMemberApplyCompleteListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                ...
                if(isLoadMore==true){
                    ...
                    adapter=new MemberCompleteInfoAdapter(mData,getActivity());
