		...
		if (App.mInstance().getNetType() != NetworkUtil.NO_NETWORK) {
			...
			try {
				...
				if (!"".equals(token) && !"".equals(time)) {
					...
					Log.i(Constant.TAG,"Authorization:"+auth.toString());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
			Log.i("test1","参数:"+ SwitchServerUtils.getJavaServerUrl() + uri + "\n" + data+ "\n");
			HttpUtils http = new HttpUtils();
			http.send(HttpMethod.POST, SwitchServerUtils.getJavaServerUrl() + uri, params, new RequestCallBack<HttpUtils>() {
				@Override
...
//					Log.i("onSuccess",
//							responseInfo.result+
//							"/n statusCode:"+responseInfo.statusCode
//							+"/n contentType:"+responseInfo.contentType
//							+"=contentEncoding:"+responseInfo.contentEncoding
//							+"/n contentLength:"+responseInfo.contentLength
//							+"/n reasonPhrase:"+responseInfo.reasonPhrase
//							+"/n hashCode:"+responseInfo.hashCode()
//							+"/n locale:"+responseInfo.locale
//							+"/n protocolVersion:"+responseInfo.protocolVersion
//							+"/n getAllHeaders:"+responseInfo.getAllHeaders().toString()
//							+"/n responseInfo:"+responseInfo
//							"/n Authorization:"+responseInfo.getHeaders("Content-Type").toString());
