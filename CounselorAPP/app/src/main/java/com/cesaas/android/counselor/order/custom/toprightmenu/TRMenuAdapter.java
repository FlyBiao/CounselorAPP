...
public class TRMenuAdapter extends RecyclerView.Adapter<TRMenuAdapter.TRMViewHolder> {
    ...
    private List<MenuItemMenu> menuItemList;
    private boolean showIcon;
    private TopRightMenu mTopRightMenu;
    private TopRightMenu.OnMenuItemClickListener onMenuItemClickListener;
    private int count;

    public TRMenuAdapter(Context context, TopRightMenu topRightMenu, List<MenuItemMenu> menuItemList, boolean show,int count) {
    ...
    public void setData(List<MenuItemMenu> data){
        menuItemList = data;
    ...
    public void onBindViewHolder(TRMViewHolder holder, int position) {
        final MenuItemMenu menuItem = menuItemList.get(position);
        if (showIcon){
        ...
        if (position == 0){
            ...
            BadgeView sessionBadgeView=new BadgeView(mContext);
            sessionBadgeView.setTargetView(holder.icon);
