package com.cesaas.android.counselor.order.viewpager.indicator;
...
public class PagerIndicator extends LinearLayout {
    ...
    private void init() {
        this.setBackgroundDrawable(getResources().getDrawable(R.drawable.indicator_bg));//indicator
        paint = new Paint();
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(getResources().getColor(R.color.color_title_bar));//滑动TextView背景色
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isSetData) {
            ...
            for (int i = 0; i < mDatas.size(); i++) {
                ...
                tv.setTextColor(getResources().getColor(R.color.color_title_bar));
                ...
                public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                    ...
                    if (isAutoSelect && currentPosition == 0) {
                        ...
                        if (positionOffset >= rebounceOffset && positionOffset < (1 - (1 - rebounceOffset) / 2)) {
                            ...
                            //当positionOffset = zero 时，设置mLeft位置
                            if (positionOffset != 0) {
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                if (i == currentPosition) {
                    //滑动选中字体色
                    ((TextView) getChildAt(currentPosition)).setTextColor(getResources().getColor(R.color.white));
                } else {
                    //默认字体色
                    ((TextView) getChildAt(i)).setTextColor(getResources().getColor(R.color.color_title_bar));
                }
            }
        }
    }
