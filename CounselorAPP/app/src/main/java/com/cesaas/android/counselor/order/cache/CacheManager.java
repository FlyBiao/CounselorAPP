...
/**
 * Author FGB
 * Description 使用内存缓存和Disk缓存双缓存的Http缓存管理类
 * Created 2017/3/16 18:56
 * Version 1.zero
 */
public class CacheManager {
     ...
     * @param strategy
     */
    public void setStrategy(CacheManager.Strategy strategy) {
        this.mStrategy = strategy;
        switch (mStrategy) {
            case MEMORY_FIRST:
                if (!mMemoryCache.hasEvictedListener()) {
                    mMemoryCache.setEvictedListener(new EvictedListener() {
                        @Override
