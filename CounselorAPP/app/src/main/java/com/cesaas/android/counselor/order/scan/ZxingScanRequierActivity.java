...
import com.cesaas.android.java.adapter.require.RequierGoodsDetailScanAdapter;
...
import com.cesaas.android.java.net.require.RequireChangeByBarcodeNoNet;
import com.cesaas.android.java.net.require.RequireDeleteByBarcodeNoNet;
import com.cesaas.android.java.net.require.RequireGoodsDetailNet;
import com.cesaas.android.java.net.require.RequireScanNet;
 ...
 *
 */
public class ZxingScanRequierActivity extends BasesActivity implements QRCodeView.Delegate ,View.OnClickListener{
	...
	private RequierGoodsDetailScanAdapter adapter;
	private List<MoveDeliveryGoodsDetailBean> mData;
	private RequireScanNet moveDeliveryScanNet;
	private RequireGoodsDetailNet receiveGetDetailListByBoxNet;
	private RequireChangeByBarcodeNoNet moveDeliveryChangeByBarcodeNoNet;
	private RequireDeleteByBarcodeNoNet moveDeliveryDeleteByBarcodeNoNet;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		requireListBean= (RequireListBean) bundle.getSerializable("RequireListBean");
	...
	public void onEventMainThread(ResultRequireGoodsDetailBean msg) {
		if(msg.arguments.resp.errorCode!=0){
			if(msg.arguments.resp.records.value!=null && msg.arguments.resp.records.value.size()!=0){
				...
				adapter=new RequierGoodsDetailScanAdapter(mData,mContext);
	...
	private void initData(){
		receiveGetDetailListByBoxNet=new RequireGoodsDetailNet(mContext);
		receiveGetDetailListByBoxNet.setData(pageIndex, FilterConditionDateUtils.getPId(requireListBean.getId()));
	}
					...
					if (materialDialog != null) {
						materialDialog.setTitle("温馨提示！")
								...
								.setPositiveButton("确定", new View.OnClickListener() {
									@Override
									public void onClick(View v) {
										moveDeliveryDeleteByBarcodeNoNet=new RequireDeleteByBarcodeNoNet(mContext);
										moveDeliveryDeleteByBarcodeNoNet.setData(requireListBean.getId(),barcodeNo);
			...
			new AsyncTask<Void, Void, String>() {
				...
				protected void onPostExecute(String result) {
					if (TextUtils.isEmpty(result)) {
						Toast.makeText(ZxingScanRequierActivity.this, "未发现二维码", Toast.LENGTH_SHORT).show();
					} else {
						//执行扫描盘点操作
						moveDeliveryScanNet=new RequireScanNet(mContext);
						moveDeliveryScanNet.setData(requireListBean.getId(),result,num);
					}
				}
			}.execute();
		}
			...
			switch (view.getId()) {
				...
				case R.id.tv_sure:
					if(!TextUtils.isEmpty(et_goods_num.getText().toString())){
						...
						moveDeliveryChangeByBarcodeNoNet=new RequireChangeByBarcodeNoNet(mContext);
						moveDeliveryChangeByBarcodeNoNet.setData(requireListBean.getId(),barcodeNo,num);
						dismiss();
					}else{
