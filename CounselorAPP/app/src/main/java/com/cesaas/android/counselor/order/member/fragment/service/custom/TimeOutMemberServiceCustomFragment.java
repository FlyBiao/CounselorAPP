...
import com.cesaas.android.counselor.order.member.adapter.service.custom.TimeOutMemberServiceCustomAdapter;
...
/**
 * Author FGB
 * Description 已超时成会员服务--会员定制
 * Created at 2018/2/27 17:33
 * Version 1.0
 */

public class TimeOutMemberServiceCustomFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener{
    ...
    private TimeOutMemberServiceCustomAdapter adapter;
     ...
     * 单例
     */
    public static TimeOutMemberServiceCustomFragment newInstance() {
        TimeOutMemberServiceCustomFragment fragmentCommon = new TimeOutMemberServiceCustomFragment();
    ...
    public void initData() {
        net=new CustomServiceListNet(getContext(),3);
    ...
    public void onEventMainThread(ResultServiceRemarkBean msg) {
        if(msg.IsSuccess!=false){
            ...
            net=new CustomServiceListNet(getContext(),1);
    ...
    public void onEventMainThread(ResultCustomServiceTimeOutListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                tv_not_data.setVisibility(View.GONE);
                mData=new ArrayList<>();
                mData.addAll(msg.TModel);
                adapter=new TimeOutMemberServiceCustomAdapter(mData,getActivity(),getContext());
