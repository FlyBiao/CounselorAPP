...
public class ViewPagerIndicator extends LinearLayout {
	...
	private static final int COUNT_DEFAULT_TAB = 3;// 默认可见tab为4个
	private List<GetCategoryBean> mTitles;// 接收传递过来的title
	private static final int COLOR_TEXT_NORMAL = Color.parseColor("#FFFFFF");
	private static final int COLOR_TEXT_HIGHLIGHT = Color.parseColor("#FF5500");
	...
	public ViewPagerIndicator(Context context, AttributeSet attrs) {
		...
		mTabVisibleCount = attributes.getInt(
				R.styleable.ViewPagerIndicator_item_count,
				COUNT_DEFAULT_TAB);
	 ...
	 * @param titles
	 */
	public void setTabItemTitles(List<GetCategoryBean> titles) {
		if (titles != null && titles.size() > 0) {
			this.removeAllViews();
			mTitles = titles;
			for (GetCategoryBean title : mTitles) {
				this.addView(generateTextView(title.Content));
			}
			setItemClickEvent();
		}
	}
