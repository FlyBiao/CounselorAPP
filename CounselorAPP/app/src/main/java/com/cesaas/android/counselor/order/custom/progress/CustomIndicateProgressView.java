...
/**
 * 自定义百分比进度条
 * Created by cebsaas on 2017/2/23.
 */

public class CustomIndicateProgressView extends View {
    ...
    private int indicatorRadius = 15; //进度指示器四个角的角度px
    private int defaultContentMargin = 2;
    private int defaultIndicateMargin = 10;
    private int max = 100;
    ...
    public CustomIndicateProgressView(Context context) {
        super(context);
        // TODO Auto-generated constructor stub
    }

    public CustomIndicateProgressView(Context context, AttributeSet attributeSet) {
...
//		progressPaint.setColor(startProgressColor);
        ...
        indicateTextPaint.setColor(getResources().getColor(R.color.cinema_list_body_bg_down));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画背景
        RectF backRectF = new RectF(0, height * 1 / 5, width, height * 3 / 5);
        backPaint.setColor(backgroundColor);
        canvas.drawRoundRect(backRectF, radius, radius, backPaint);

        //画进度
        RectF progressRectF = new RectF(0, height * 1 / 5, width * getScale(), height * 3 / 5);
...
//		indicateTextPaint.setTextSize(height * 2 / 5);
        ...
        RectF indicatorRectF = new RectF(left, height / 2, right, height * 2 / 5);
        indicateBackPaint.setColor(indicateTextColor);
        ...
        RectF indicatorContentRectF = new RectF(left + defaultContentMargin, height / 5 + defaultContentMargin,
                right - defaultContentMargin, height * 2 / 5 - defaultContentMargin);
        indicateBackPaint.setColor(getResources().getColor(R.color.colorAccent2));
        canvas.drawRoundRect(indicatorContentRectF, indicatorRadius, indicatorRadius, indicateBackPaint);

        //画指示器文本
        indicateTextPaint.setTextSize(height * 2 / 6);
