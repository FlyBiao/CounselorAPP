...
public class CompleteTaskListAdapter extends BaseQuickAdapter<ShopTaskListBean, BaseViewHolder> {
    ...
    public CompleteTaskListAdapter(List<ShopTaskListBean> mData,Activity activity) {
        super( R.layout.item_task_complete,mData);
        this.mData=mData;
    ...
    protected void convert(BaseViewHolder helper, final ShopTaskListBean item) {
        ...
        helper.setOnClickListener(R.id.ll_handle_task, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                BottonDialogUtils.showBottonTaskDialog(mContext,mActivity,item,2);
            }
        });
    }
}
