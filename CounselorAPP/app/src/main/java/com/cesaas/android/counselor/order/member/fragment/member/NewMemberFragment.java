...
import com.cesaas.android.counselor.order.member.adapter.member.NewMemberAdapter;
...
public class NewMemberFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener,SwipeRefreshLayout.OnRefreshListener,NewMemberAdapter.MyOnItemClickListener{

    private RecyclerView mRecyclerView;
    private SwipeRefreshLayout mSwipeRefreshLayout;
    private TextView tv_not_data,tv_new_member_sum,tv_select_o;
    ...
    public static NewMemberFragment newInstance() {
        NewMemberFragment fragmentCommon = new NewMemberFragment();
    ...
    public void onEventMainThread(SearchVipEventBean msg) {
        if(msg.getType()==30){
            ...
            net.setData(pageIndex,2,msg.getContent());
        }
    }

    public void onEventMainThread(ResultQueryVipBean msg) {
        if(msg!=null && msg.getMemberType()==30){
...
//            bean.setFilters(queryVipBean.getVipInfo());
    ...
    public void onEventMainThread(ResultSetRemarkBean msg) {
        if(MemberType==3){
            if(msg.IsSuccess!=false){
                ...
                net=new MemberServiceListNet(getContext(),3);
                net.setData(pageIndex,2);
            }else{
    ...
    public void onEventMainThread(ResultFocusBean msg) {
        if(msg.IsSuccess!=false){
            if(FocusType==1){
                ...
                net=new MemberServiceListNet(getContext(),3);
                net.setData(pageIndex,2);net=new MemberServiceListNet(getContext(),1);
    ...
    public void onEventMainThread(ResultNewMemberServiceListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                totalSize=msg.RecordCount;
                tv_new_member_sum.setText(msg.RecordCount+"");
                ...
                if(isLoadMore==true){
                    ...
                    adapter=new NewMemberAdapter(mData,getActivity(),getContext());
                    adapter.notifyDataSetChanged();
    ...
    public void initViews() {
        ...
        tv_new_member_sum=findView(R.id.tv_new_member_sum);
