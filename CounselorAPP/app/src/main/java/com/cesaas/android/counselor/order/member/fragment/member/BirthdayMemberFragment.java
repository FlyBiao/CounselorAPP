...
import com.cesaas.android.counselor.order.member.adapter.member.BirthdayMemberAdapter;
...
import com.cesaas.android.counselor.order.member.bean.service.check.QueryVipBean;
...
public class BirthdayMemberFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener,SwipeRefreshLayout.OnRefreshListener,BirthdayMemberAdapter.MyOnItemClickListener{

    private RecyclerView mRecyclerView;
    private SwipeRefreshLayout mSwipeRefreshLayout;
    private TextView tv_not_data,tv_birthday_member_sum,tv_select_o;
    private LinearLayout ll_cancel_check,ll_show_switch;
    ...
    private BirthdayMemberAdapter adapter;
     ...
     * 单例
     */
    public static BirthdayMemberFragment newInstance() {
        BirthdayMemberFragment fragmentCommon = new BirthdayMemberFragment();
    ...
    public void onEventMainThread(SearchVipEventBean msg) {
        if(msg.getType()==20){
            ...
            net.setData(pageIndex,1,msg.getContent());
        }
    }

    public void onEventMainThread(ResultQueryVipBean msg) {
        if(msg!=null && msg.getMemberType()==20){
...
//            QueryVipBean bean=new QueryVipBean();
//            bean.setFilters(queryVipBean.getVipInfo());

            net=new MemberServiceListNet(getContext(),1);
            net.setData(pageIndex,1,queryVipBean.getVipInfo());
        }
    }

    public void onEventMainThread(RemarkEventBusBean msg) {
        MemberType=msg.getMemberType();
        if(msg.getMemberType()==2){
    ...
    public void onEventMainThread(ResultSetRemarkBean msg) {
        if(MemberType==2){
            if(msg.IsSuccess!=false){
                ...
                net=new MemberServiceListNet(getContext(),2);
                net.setData(pageIndex,1);
            }else{
    ...
    public void onEventMainThread(ResultBirthdayMemberServiceListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                totalSize=msg.RecordCount;
                tv_birthday_member_sum.setText(msg.RecordCount+"");
                ...
                if(isLoadMore==true){
                    ...
                    adapter=new BirthdayMemberAdapter(mData,getActivity(),getContext());
                    adapter.notifyDataSetChanged();
    ...
    public int getLayoutId() {
        return R.layout.fragment_new_birthday_member;
    }
    ...
    public void initViews() {
        ...
        tv_birthday_member_sum=findView(R.id.tv_birthday_member_sum);
