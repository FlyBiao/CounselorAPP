...
public class CustomDirectSellingCenterProgress extends View {

    private static final int DEFAULT_MIN_WIDTH = 200; //View默认大小
    //圆弧的宽度
    private static final float DEFAULT_BORDER_WIDTH = 30f;
    //小原点的半径
    private int  DEFAULT_LITLE_WIDTH = dipToPx(10);

    private ValueAnimator progressAnimator;
    private float textSize = dipToPx(15);
    private float BIM_textSize = dipToPx(14);
    private float Number_textSize = dipToPx(20);
    ...
    DecimalFormat df = new DecimalFormat("zero.zero");//格式化小数，不足的补0
    //分段颜色
//    private int[] colors = new int[]{Color.GREEN, Color.YELLOW, Color.RED, Color.RED};
    private int[] colors = new int[]{getResources().getColor(R.color.colorAccent5),
            getResources().getColor(R.color.colorAccent4),
            getResources().getColor(R.color.colorAccent3),
            getResources().getColor(R.color.cinema_yellow_2),
     ...
     * 填充色主要参数：
     ...
     18.5/24.zero/28.zero/35.zero
     */
    private float positions[]=new float[colors.length];
    //分割区域的数值
    private double position_line[] = new double[]{20,30,40,50,60};

    public CustomDirectSellingCenterProgress(Context context) {
        super(context);
    }

    public CustomDirectSellingCenterProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paintDefalt.setColor(getResources().getColor(R.color.colorAccent2));
...
//        vTextPaint.setStrokeWidth((float) 3.zero);
...
//        canvas.drawText("胖", getWidth()-marging, centerX+marging+textSize+padding, vTextPaint);
        ...
        canvas.drawText("直销", getWidth()/2, (int)((centerX+marging)/3*1.5+textSize), vTextPaint);
...
//        canvas.drawCircle(zero, zero, DEFAULT_LITLE_WIDTH, paintCircle);//这时候的画布已经移动到了中心位置
        /**
         * 第一个参数为正则顺时针，否则逆时针
         * 后面两个参数是圆心
         * 画布的旋转一定要在，画图形之前进行旋转
