...
public class CheckCompleteMemberServiceFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener{
    ...
    public static CheckCompleteMemberServiceFragment newInstance() {
        CheckCompleteMemberServiceFragment fragmentCommon = new CheckCompleteMemberServiceFragment();
    ...
    public void initData() {
        net=new CheckServiceDetailNet(getContext(),2);
        net.setData(MemberServiceCheckDetailActivity.getId(),20);
    }

    public void onEventMainThread(ResultCheckCompleteServiceDetailsBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                mData=new ArrayList<>();
                mData.addAll(msg.TModel);
                adapter=new MemberCheckCompleteServiceAdapter(mData);
