package com.cesaas.android.counselor.order.member.fragment.service.visit;
...
public class TimeOutMemberServiceFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener{
    ...
    private TimeOutMemberServiceVisitAdapter adapter;
    private List<SalesVisitServiceBean> mData=new ArrayList<>();
    private VisitListNet net;
     ...
     * 单例
     */
    public static TimeOutMemberServiceFragment newInstance() {
        TimeOutMemberServiceFragment fragmentCommon = new TimeOutMemberServiceFragment();
    ...
    public void initData() {
        net=new VisitListNet(getContext(),3);
    ...
    public void onEventMainThread(ResultServiceRemarkBean msg) {
        if(msg.IsSuccess!=false){
            ...
            net=new VisitListNet(getContext(),1);
    ...
    public void onEventMainThread(ResultVisitServiceTimeOutListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                tv_not_data.setVisibility(View.GONE);
                mData=new ArrayList<>();
                mData.addAll(msg.TModel);
                adapter=new TimeOutMemberServiceVisitAdapter(mData,getActivity(),getContext());
