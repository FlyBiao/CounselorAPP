...
import com.cesaas.android.counselor.order.member.adapter.service.apply.MemberTimeOutInfoAdapter;
...
/**
 * Author FGB
 * Description 完成会员更改信息
 * Created at 2018/2/27 17:33
 * Version 1.0
 */

public class TimeOutMemberInfoFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,BaseQuickAdapter.RequestLoadMoreListener{
    ...
    private MemberTimeOutInfoAdapter adapter;
    private List<MemberApplyListBean> mData=new ArrayList<>();

    /**
     * 单例
     */
    public static TimeOutMemberInfoFragment newInstance() {
        TimeOutMemberInfoFragment fragmentCommon = new TimeOutMemberInfoFragment();
        return fragmentCommon;
    }

    @Override
    public int getLayoutId() {
        return R.layout.fragment_timeout_member_info;
    }
    ...
    public void initData() {
        net=new VipDataListNet(getContext(),4);
    ...
    public void onEventMainThread(SearchVipEventBean msg) {
        if(msg.getType()==40){
            ...
            net.setData(pageIndex,40,msg.getContent());
        }
    }

    public void onEventMainThread(ResultMemberApplyTimeOutListBean msg) {
        if(msg.IsSuccess!=false){
            if(msg.TModel!=null && msg.TModel.size()!=0){
                ...
                if(isLoadMore==true){
                    ...
                    adapter=new MemberTimeOutInfoAdapter(mData,getActivity());
