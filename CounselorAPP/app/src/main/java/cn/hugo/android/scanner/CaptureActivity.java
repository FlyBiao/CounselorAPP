...
public final class CaptureActivity extends Activity implements
	...
	private String intentVipCode;
	private String intentShopCode;
	private String CheckCargoResultCode;
	private String ScanCheckCargo;
	private String scanMainResultCode;
	private String mScanCheckActivityResult;
	private String mPosActivityResult;
	private String maddScanSendActivityResult;
	private String TryWearCode;
	private String mSendScanSendActivityResult;
	...
	private LinearLayout ll_default_scan;
	private LinearLayout ll_main_scan;
	private LinearLayout ll_goods,ll_member,ll_activity;
	private ImageView iv_goods,iv_member,iv_activity;
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        TicketCode = intent.getStringExtra("intentCode"); 
        intentVipCode=intent.getStringExtra("intentVipCode");
        intentShopCode=intent.getStringExtra("intentShopCode");
        CheckCargoResultCode=intent.getStringExtra("CheckCargoResultCode");
        ScanCheckCargo=intent.getStringExtra("scanCheckCargoResultCode");
        mScanCheckActivityResult=intent.getStringExtra("mScanCheckActivityResult");
        mPosActivityResult=intent.getStringExtra("mPosActivityResult");
        maddScanSendActivityResult=intent.getStringExtra("maddScanSendActivityResult");
		TryWearCode=intent.getStringExtra("TryWearCode");
		mSendScanSendActivityResult=intent.getStringExtra("mSendScanSendActivityResult");
		scanMainResultCode=intent.getStringExtra("scanMainResultCode");
		...
		ll_default_scan=(LinearLayout) findViewById(R.id.ll_default_scan);
		ll_main_scan=(LinearLayout) findViewById(R.id.ll_main_scan);
		if(scanMainResultCode!=null && scanMainResultCode.equals("013")){
			ll_main_scan.setVisibility(View.VISIBLE);
			ll_default_scan.setVisibility(View.GONE);
		}else{
			ll_default_scan.setVisibility(View.VISIBLE);
			ll_main_scan.setVisibility(View.GONE);
		}
		...
		ll_goods.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				scanType=1;
				iv_goods.setImageResource(R.mipmap.is_scan);
				iv_member.setImageResource(R.mipmap.not_scan);
				iv_activity.setImageResource(R.mipmap.not_scan);
			}
		});ll_member.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				scanType=2;
				iv_goods.setImageResource(R.mipmap.not_scan);
				iv_member.setImageResource(R.mipmap.is_scan);
				iv_activity.setImageResource(R.mipmap.not_scan);
			}
			...
			public void onClick(View v) {
				...
				iv_goods.setImageResource(R.mipmap.not_scan);
				iv_member.setImageResource(R.mipmap.not_scan);
				iv_activity.setImageResource(R.mipmap.is_scan);
			}
		});
...
//			ScanResultActivity.activity.finish();
	...
	public void handleDecode(Result rawResult, Bitmap barcode, float scaleFactor) {
		...
		if(ResultParser.parseResult(rawResult).toString()!=null){
			if(TicketCode!=null){
				if(TicketCode.equals("001")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("TicketCode","001");
				}
			}
			if(mSendScanSendActivityResult!=null){
				if(mSendScanSendActivityResult.equals("101")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("TryWearCode","101");
				}
			}
			if(TryWearCode!=null){
				if(TryWearCode.equals("112")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("TryWearCode","112");
				}
			}
			if(intentVipCode!=null){
				if(intentVipCode.equals("004")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("intentVipCode","004");
					Log.i(TAGS, "扫描条码:"+ResultParser.parseResult(rawResult).toString());
				}
			}
			if(CheckCargoResultCode!=null){
				if(CheckCargoResultCode.equals("007")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("CheckCargoResultCode","007");
					Log.i(TAGS, "扫描条码:"+ResultParser.parseResult(rawResult).toString());
				}
			}
			if(ScanCheckCargo!=null){
				if(ScanCheckCargo.equals("008")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("ScanCheckCargo","008");
					Log.i(TAGS, "扫描条码:"+ResultParser.parseResult(rawResult).toString());
				}
			}
			if(mScanCheckActivityResult!=null){
				if(mScanCheckActivityResult.equals("009")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("mScanCheckActivityResult","009");
					Log.i(TAGS, "扫描条码:"+ResultParser.parseResult(rawResult).toString());
				}
			}
			if(intentShopCode!=null){
				if(intentShopCode.equals("002")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("intentShopCode","002");
					Log.i(TAGS, "扫描条码:"+ResultParser.parseResult(rawResult).toString());
				}
			}
			if(mPosActivityResult!=null){
				if(mPosActivityResult.equals("011")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("mPosActivityResult","011");
				}
			}
			if(maddScanSendActivityResult!=null){
				if(maddScanSendActivityResult.equals("012")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
					intent.putExtra("maddScanSendActivityResult","012");
				}
			}if(scanMainResultCode!=null){
				if(scanMainResultCode.equals("013")){
					intent.putExtra("resultCode",ResultParser.parseResult(rawResult).toString());
	 ...
	 * @param scaleFactor
	 */
	public void scanResultcode(Result rawResult, Bitmap barcode, float scaleFactor) {
